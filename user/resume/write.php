<?php
    ...
    if(isset($_POST['status']) && $_POST['status'] === "save" || isset($_POST['final'])){
        ...
        if(isset($_GET['idx'])){
            ...
            $query = sprintf("update resume set realname='%s',email='%s',title='%s',contents='%s',special='%s' where idx='%s';",$realname, $email, $title, $contents,$special,$idx);
            $result = mysqli_query($conn, $query);
        ...
        //
            ...
            if($now_resume_count > 3){
                ...
                $query = sprintf("insert into resume (username, realname,email, title, contents , special) values 
                        ('%s', '%s', '%s','%s', '%s', '%s');",
                        $username, $realname, $email, $title, $contents ,$special );
                $result = mysqli_query($conn, $query);
...
<select class="select"id="job" name="point" required>
    <?php 
    if(restore('speical') !== "" ){
        echo "<option value='".restore('speical')."' selected>".restore('special')."</option>";
    }
    ?>
